...
pub async fn compress_sse(request: Request, next: Next) -> Response {
    ...
    let (Some(ct), Some(ae)) = (content_type, accept_encoding) else { return response; };
    if content_encoding.is_some() || ct.as_bytes() != b"text/event-stream" { return response; }
    if !ae.to_str().map(|v| v.contains("gzip")).unwrap_or(false) { return response; }
...
struct CompressedStream { inner: BodyDataStream, compression: GzEncoder<Vec<u8>> }
impl CompressedStream {
    pub fn new(body: BodyDataStream) -> Self {
        Self { inner: body, compression: GzEncoder::new(Vec::new(), Compression::default()) }
    }
}
