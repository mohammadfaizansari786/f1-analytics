...
use axum::{extract::State, response::{sse, Sse}};
...
pub async fn sse_handler(State(state): State<Arc<AppState>>) -> Sse<impl Stream<Item = Result<sse::Event, Infallible>>> {
    ...
    let initial_stream = futures::stream::once(async {
        ...
        Ok(sse::Event::default().event("initial").json_data(initial_state).unwrap())
    });
    ...
    let keep_alive = sse::KeepAlive::new().interval(Duration::from_secs(10)).text("keep-alive-text");
