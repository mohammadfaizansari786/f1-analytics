...
pub async fn init() -> Result<WsStream, Box<dyn Error>> {
    ...
    let connect_result = timeout(Duration::from_secs(10), tokio_tungstenite::connect_async(req)).await;
    ...
    info!("connected");
    socket.send(tungstenite::Message::text(consts::SIGNALR_SUBSCRIBE)).await?;
    info!("subscribed");
    Ok(socket)
}
    ...
    match env_url() {
        ...
        None => {
            ...
            let url = Url::parse_with_params(
                ...
                &[("clientProtocol", "1.5"), ("transport", "webSockets"), ("connectionToken", &negotiation.token), ("connectionData", consts::SIGNALR_HUB)],
            )?;
            ...
            headers.insert(header::COOKIE, negotiation.cookie.parse().unwrap());
            Ok(req)
        }
    }
}

struct Negotiaion { token: String, cookie: String }

async fn negotiate() -> Result<Negotiaion, Box<dyn Error>> {
    trace!("negotiating");
    let url = Url::parse_with_params(
        &format!("https://{}/negotiate", consts::F1_BASE_URL),
        &[("clientProtocol", "1.5"), ("connectionData", consts::SIGNALR_HUB)],
    )?;
